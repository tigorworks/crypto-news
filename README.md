...
2. **Setelah revisi, hanya kesalahan ANGKA yang masih boleh menahan** — termasuk kalau langkah revisinya sendiri GAGAL (error LLM, anggaran token habis, balasan di luar struktur). Temuan non-angka yang bertahan sampai putaran kedua diturunkan jadi tanda editorial dan analisanya tetap terkirim. Ini keputusan sadar berdasarkan bukti produksi: temuan `pengetahuan_luar` yang bertahan sampai putaran kedua hampir selalu salah kategori — kalimat tafsir yang divonis fakta karangan — dan menahan SELURUH analisa (narasi + outlook + skenario sekaligus) karena satu kalimat tafsir jauh lebih merugikan pembaca. Angka karangan tetap menahan tanpa pengecualian, karena angka yang salah menyesatkan secara langsung dan kode sudah memverifikasinya sendiri.
...
**Pembantah ketiga: keberatan soal ISTILAH.** Pola yang bertahan sampai putaran kedua di produksi berbunyi *"Istilah pertemuan Gedung Putih tidak muncul dalam ringkasan berita"* — keberatan soal PILIHAN KATA, bukan keberadaan fakta, padahal entitasnya jelas ada di judul berita yang disuplai. Kode sekarang mengenali bentuk itu (*istilah/frasa/sebutan/terminologi … tidak muncul/ada/ditemukan*) dan membantahnya HANYA kalau setiap entitas multi-kata pada kutipan (`"Gedung Putih"`, `"Federal Reserve"`) memang ditemukan di teks data. Entitas satu kata sengaja tidak dihitung: kata pertama kalimat juga berhuruf besar, dan mencocokkannya akan membatalkan tuduhan yang justru benar. Tuduhan tentang PERISTIWA karangan yang menyangkut entitas nyata (*"Gedung Putih mengumumkan pembelian 200.000 BTC"*) tidak ikut terbantah, karena alasannya tidak berbicara soal istilah.
...
| *Bagian AI yang ditahan critic: narasi, outlook, outlook_skenario* | Pelonggaran setelah revisi cuma bisa dicapai kalau langkah revisi **menghasilkan teks**. Kalau revisinya sendiri gagal, seluruh temuan fatal berdiri utuh dan SEMUA bagian AI ditahan — kegagalan di pihak kita menghapus analisa yang isinya belum tentu salah | Pelonggaran kini berlaku begitu revisi **dicoba**, berhasil atau tidak. Ditambah pembantah kode ketiga untuk keberatan bergaya "istilah ini tidak muncul di data" |
...
- **Kartu AGENDA BESAR 3 HARI KE DEPAN paling atas** — slot teratas halaman, DI ATAS harga dan skor sentimen, berisi **satu** agenda saja: yang dampaknya ke kripto paling besar dalam tiga hari mendatang, lengkap dengan hitung mundur, waktu WIB, dan rantai transmisinya ke BTC (`jalur`). Sengaja satu, bukan daftar — daftar lengkapnya sudah ada di bagian Agenda, dan slot teratas ini gunanya justru memaksa satu hal saja yang diingat.
  Ambangnya `relevansi_kripto >= 4` ("dampak besar"), sama dengan ambang filter agenda dan notice <24 jam: kalau "besar" berarti hal berbeda di tiap tempat, pembaca tidak bisa mempercayai satu pun.
  Urutannya relevansi kripto → bobot dampak ekonomi → waktu. Yang ditanyakan adalah dampak TERBESAR, bukan yang terdekat, jadi FOMC Minutes tiga hari lagi tetap menang atas rilis kelas menengah besok.
  **Jendelanya dihitung per HARI KALENDER WIB, bukan 72 jam mentah.** Bukan detail sepele: brief terbit sekitar 00:30 WIB, dan FOMC Meeting Minutes tiga hari kemudian jatuh di `jam_lagi` 72,6 — lewat 36 menit dari batas 72 jam, padahal siapa pun yang membaca "3 hari ke depan" jelas mengharapkannya muncul. Ini tertangkap saat pengujian dengan data produksi, bukan diperkirakan.
  Warnanya mengikuti KEDEKATAN waktu (merah <24 jam, kuning selebihnya), bukan besar dampaknya — yang lolos ke kartu ini semuanya sudah berdampak besar, jadi yang membedakan tinggal seberapa mendesak.
  Kalau tidak ada yang lolos ambang, kartunya tetap tampil dan menyatakan jalan tiga hari ke depan lapang. Itu informasi pasar tersendiri, bukan slot kosong.
...
├── collectors/
│   ├── binance.py          # harga, klines, funding, OI (+ fallback OKX lalu CoinGecko)
...
| Log penuh "HTTP 451 restricted location" | Normal di GitHub Actions. Binance menolak IP runner yang berbasis AS — pembatasan wilayah permanen. Harga otomatis pindah ke OKX (CoinGecko jadi lapis ketiga), funding/OI ke Bybit. |
